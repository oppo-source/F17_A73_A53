...
#ifdef VENDOR_EDIT
/*Le.Li@PSW.MM.AudioDriver.Codec, 2020/10/12, Add for voice noise issue event */
...
#ifdef VENDOR_EDIT
/*Le.Li@PSW.MM.AudioDriver.ADSP, 2020/09/16, Modify for Qcom test SBA to get dump of voice noise issue */
...
static int32_t qdsp_cvp_callback(struct apr_client_data *data, void *priv)
{
	...
	/*Le.Li@PSW.MM.AudioDriver.ADSP, 2020/09/16, Modify for Qcom test SBA to get dump of voice noise issue */
		...
		/*Le.Li@PSW.MM.AudioDriver.Codec, 2020/10/12, Add for voice noise issue event */
		scnprintf(pkg_str, sizeof(pkg_str), "%s: vss_ivocproc_ssr_cnt = %d!\n", __func__, common.vss_ivocproc_ssr_cnt);
		scnprintf(fb_str, sizeof(fb_str), "NULL$$EventID@@%d$$EventData@@%d$$PackageName@@%s",
			OPPO_MM_DIRVER_FB_EVENT_ID_VOC_SSR_ADSP_RESET, common.vss_ivocproc_ssr_cnt, pkg_str);
		upload_mm_kevent_feedback_data(OPPO_MM_DIRVER_FB_EVENT_MODULE_AUDIO, fb_str);
	}
	#endif /* VENDOR_EDIT */
