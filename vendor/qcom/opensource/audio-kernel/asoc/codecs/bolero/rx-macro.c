		...
		for (j = 0; j < INTERP_MAX; j++) {
			...
			if ((inp0_sel == int_1_mix1_inp + INTn_1_INP_SEL_RX0) ||
				...
				int_fs_reg = BOLERO_CDC_RX_RX0_RX_PATH_CTL +
					     0x80 * j;
				pr_err("%s: AIF_PB DAI(%d) connected to INT%u_1\n",
					  __func__, dai->id, j);
				pr_err("%s: set INT%u_1 sample rate to %u\n",
					__func__, j, sample_rate);
				/* sample_rate is in Hz */
				snd_soc_component_update_bits(component,
						int_fs_reg,
						0x0F, rate_reg_val);
				pr_err("%s:TEST: sample_rate setting RX main path of reg:0x%x val:0x%x\n",
					__func__, int_fs_reg, snd_soc_component_read32(component, int_fs_reg));
			}
			int_mux_cfg0 += 8;
		}
	}
		...
		for (j = 0; j < INTERP_MAX; j++) {
			...
			if (int_mux_cfg1_val == int_2_inp +
				...
				int_fs_reg = BOLERO_CDC_RX_RX0_RX_PATH_MIX_CTL +
						0x80 * j;
				pr_err("%s: AIF_PB DAI(%d) connected to INT%u_2\n",
					  __func__, dai->id, j);
				pr_err("%s: set INT%u_2 sample rate to %u\n",
					__func__, j, sample_rate);
				snd_soc_component_update_bits(
						component, int_fs_reg,
						0x0F, rate_reg_val);
				pr_err("%s:TEST: sample_rate setting RX mix path of reg:0x%x val:0x%x\n",
					__func__, int_fs_reg, snd_soc_component_read32(component, int_fs_reg));
			}
			int_mux_cfg1 += 8;
		}
				 ...
				 bool mclk_enable, bool dapm)
{
	...
	dev_err(rx_priv->dev, "%s: mclk_enable = %u, dapm = %d clk_users= %d\n",
	...
	if (mclk_enable) {
		...
		dev_err(rx_priv->dev, "TEST: %s: RX enable seq CLK_ID requested:%d\n",
			__func__, rx_priv->clk_id);
		...
		if (rx_priv->rx_mclk_users == 0) {
			...
			bolero_clk_rsc_fs_gen_request(rx_priv->dev,
			   false);
		dev_err(rx_priv->dev, "TEST: %s: RX disable_seq CLK_ID requested:%d\n",
			__func__, rx_priv->clk_id);
