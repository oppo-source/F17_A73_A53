...
//#ifdef ODM_HQ_EDIT
/* dongfeiju, 2020/06/26, Add for TP compatible ic 36525c*/
...
static void store_to_file(int fd, char *format, ...)
{
    ...
    if(fd >= 0) {
        ksys_write(fd, buf, strlen(buf));//by wenjie
    }
}
...
TEST_END:
    ...
    ksys_mkdir("/sdcard/TpTestReport/screenOff/NG", 0666);//by wenjie
    fd = ksys_open(data_buf, O_WRONLY | O_CREAT | O_TRUNC, 0);//by wenjie
    if (fd < 0) {
...
TEST_END:
    ...
    ksys_mkdir("/sdcard/TpTestReport/screenOn/NG", 0666);//by wenjie
    nvt_testdata->fd = ksys_open(data_buf, O_WRONLY | O_CREAT | O_TRUNC, 0);//by wenjie
...
OUT:
    if (nvt_testdata->fd >= 0) {
        ksys_close(nvt_testdata->fd);//by wenjie
    }
    set_fs(old_fs);
