...
#ifdef VENDOR_EDIT
/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2020/05/29,
		...
		struct plane_state *pstates, int cnt)
{
	...
	if (dimlayer_hbm || dimlayer_bl) {
		...
		if (((get_oppo_display_scene() == OPPO_DISPLAY_AOD_SCENE) || \
			(get_oppo_display_scene() == OPPO_DISPLAY_AOD_HBM_SCENE)) && \
			(fp_index > 0) && oppo_aod_mode){
				cstate->fingerprint_dim_layer = NULL;
		} else {
			if (aod_index >= 0) {
			...
			if (sde_crtc_config_fingerprint_dim_layer(&cstate->base, zpos)) {
				...
				/* liuzhizun, 2020/07/27, modify for power on hbm*/
				 !oppo_aod_mode &&(oppo_dimlayer_hbm_count < 0)) {
				#endif
					oppo_underbrightness_alpha = 0;
					cstate->fingerprint_dim_layer = NULL;
					cstate->fingerprint_mode = false;
					cstate->fingerprint_pressed = false;
					oppo_dimlayer_hbm_count++;
					return 0;
				}
				SDE_EVT32(zpos, fp_index, aod_index, fppressed_index, cstate->num_dim_layers);
				return -EINVAL;
