...
struct wakeup_source wakeup_source_egistec_fp;
...
static irqreturn_t fp_eint_func_ll(int irq , void *dev_id)
{
	...
	__pm_wakeup_event(&wakeup_source_egistec_fp, msecs_to_jiffies(1500));
 ...
 static void egistec_power_control(struct etspi_data *etspi, int power_onoff)
{
	...
	if (power_onoff) {
		//gpio_set_value(etspi->vcc_Pin, 1); /*set the power on*/
		gpio_direction_output(etspi->vcc_Pin, 1);
	}
                
	else {
		//gpio_set_value(etspi->vcc_Pin, 0);  /*set the power off*/
...
static long etspi_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case FP_WAKELOCK_TIMEOUT_DISABLE: //0Xb2
		...
		__pm_relax(&wakeup_source_egistec_fp);
		goto done;
	case FP_POWER_ONOFF:
...
static int etspi_remove(struct platform_device *pdev)
{
	...
	//wakeup_source_destroy(&wakeup_source_egistec_fp);
	wakeup_source_trash(&wakeup_source_egistec_fp);
	request_irq_done = 0;
	/* t_mode = 255; */
	return 0;
}
...
//	struct regulator *vreg;
	...
	if(qpnp_is_power_off_charging()){
		pr_err("in charge mode no need fingerprint");
		return status;
	}
...
*/
	...
	fps_ints.drdy_irq_flag = DRDY_IRQ_DISABLE;
	wakeup_source_init(&wakeup_source_egistec_fp, "egis_wakeup");
