...
struct tx_macro_priv {
	...
	#ifdef VENDOR_EDIT
	/*Yuchun.Yang@PSW.MM.AudioDriver.Codec, 2020/10/27, Add for resolve glitch during amic record*/
				...
				bool mclk_enable)
{
	...
	dev_err(tx_priv->dev, "%s: mclk_enable = %u,clk_users= %d\n",
			   ...
			   struct snd_soc_dai *dai)
{
	...
	for_each_set_bit(decimator, &tx_priv->active_ch_mask[dai->id],
		...
		if (decimator >= 0) {
			tx_fs_reg = BOLERO_CDC_TX0_TX_PATH_CTL +
				    TX_MACRO_TX_PATH_OFFSET * decimator;
			dev_err(component->dev, "%s: set DEC%u rate to %u\n",
				__func__, decimator, sample_rate);
			snd_soc_component_update_bits(component, tx_fs_reg,
						0x0F, tx_fs_rate);
			dev_err(component->dev, "%s: set DEC rate reg:0x%x val: 0x%x\n",
				__func__, tx_fs_reg, snd_soc_component_read32(component, tx_fs_reg));
		} else {
			dev_err(component->dev,
