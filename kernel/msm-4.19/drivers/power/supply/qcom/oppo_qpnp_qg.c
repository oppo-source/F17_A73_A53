...
#ifdef VENDOR_EDIT
/* zhangkun@BSP.CHG.Basic, 2020/04/22, Add for oppo_gauge*/
...
int qg_get_battery_current(struct qpnp_qg *chip, int *ibat_ua)
{
	...
	*ibat_ua = qg_iraw_to_ua(chip, last_ibat);
	...
	#ifdef ODM_HQ_EDIT
	/* Yuzhe.Peng@ODM.BSP.CHARGE 2020/07/26 Add global ibat for battery temperture compensation */
	ibat_ma_adc_comp = *ibat_ua / 1000;
	pr_err("ibat_ma_adc_comp = %d\n", ibat_ma_adc_comp);
	#endif
...
static void qg_notify_charger(struct qpnp_qg *chip)
{
	...
	if (!chip->profile_loaded)
		return;
#ifdef ODM_HQ_EDIT
	/* wangxianfei@ODM.BSP.charge, Fcv only can be setted by oppochg_set_fv,but no battery profile max-voltage-uv */
#else
...
static bool is_batt_id_valid(struct qpnp_qg *chip)
{
	...
	if (project_num == 20021 || project_num == 20221 || project_num == 20222 ||
		project_num == 20223 || project_num == 20224 || project_num == 20225 ||
		project_num == 20226 || project_num == 20227 || project_num == 20228 ||
		project_num == 20229 || project_num == 20202 || project_num == 20203 ||
		project_num == 20204 || project_num == 20207 || project_num == 20208) {
		proj_batterys.batt_info = soda_batt_info;
		proj_batterys.batt_vendor_count = sizeof(soda_batt_info) / sizeof(struct batt_info);
	} else if (project_num == 20211 || project_num == 20212 || project_num == 20213 ||
			project_num == 20214 || project_num == 20215) {
...
static int oppo_get_healtd_ready(void)
{
...
static struct oppo_gauge_operations qonp_gauge_ops = {
	...
	.get_healtd_ready = oppo_get_healtd_ready,
};
#endif
