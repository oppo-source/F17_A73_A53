...
int dsi_display_cabc_high_mode(struct dsi_display *display) {
	...
	rc = dsi_panel_cabc_high_mode(display->panel);
		if (rc) {
			pr_err("[%s] failed to dsi_display_cabc_high_mode, rc=%d\n",
			       display->name, rc);
	}
...
int oppo_display_get_cabc_mode(void)
{
	return cabc_mode;
}
...
struct device_attribute *attr, char *buf) {
	...
	if (display->panel->panel_type != DSI_DISPLAY_PANEL_TYPE_OLED) {
		printk(KERN_INFO "[lcm] oppo_display_get_panel_serial_number only support oled!\n");
		return -1;
	}
...
extern int oppo_dimlayer_hbm_count;
