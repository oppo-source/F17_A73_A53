	...
	if (enable) {
		if (priv->reg_seq_en_cnt++ == 0) {
			for (i = 0; i < (priv->num_fs_reg * 2); i += 2) {
				dev_err(priv->dev, "%s: Register: %d, value: %d\n",
				...
				bool enable)
{
	...
	dev_err(priv->dev, "%s: clk_cnt: %d for requested clk: %d, enable: %d\n",
...
static int bolero_clk_rsc_probe(struct platform_device *pdev)
{
	...
	dev_err(&pdev->dev, "%s: num_fs_reg %d\n", __func__, priv->num_fs_reg);
	...
	for (i = 0; i < MAX_CLK; i++) {
		...
		for (j = 0; j < clk_cnt; j++) {
			if (!strcmp(clk_src_name[i], clk_name_array[j])) {
				...
				priv->clk[i] = clk;
				dev_err(&pdev->dev, "%s: clk get success for clk name %s\n",
	...
	if (ret) {
		dev_err(&pdev->dev, "%s: could not find qcom,rx_mclk_mode_muxsel entry in dt\n",
			__func__);
	} else {
	...
	if (ret) {
		dev_err(&pdev->dev, "%s: could not find qcom,wsa_mclk_mode_muxsel entry in dt\n",
			__func__);
	} else {
	...
	if (ret) {
		dev_err(&pdev->dev, "%s: could not find qcom,va_mclk_mode_muxsel entry in dt\n",
			__func__);
	} else {
