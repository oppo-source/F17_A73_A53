...
static int get_buffer_internal(AVCodecContext *avctx, AVFrame *frame, int flags)
{
    ...
    //Yugang.Ma@PSW.MM.MediaServer.Player, 2020/04/04, Add for CTS test fail:testStagefright_bug_34360591
    if ((avctx->codec_type == AVMEDIA_TYPE_VIDEO) && (avctx->codec_id == AV_CODEC_ID_VP9)) {
        if ((avctx->width * avctx->height) > (4096 * 4096)) {
            av_log(avctx, AV_LOG_ERROR, "resolution is too large\n");
            return AVERROR(EINVAL);
        }
    }
