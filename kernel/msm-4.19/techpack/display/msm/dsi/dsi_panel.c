...
extern int oppo_bl_normal_max_level;
int oppo_dfps_idle_off = 0;

#endif /*VENDOR_EDIT*/
//#ifdef ODM_HQ_EDIT
/* dongfeiju, 2020/05/13, Add for TP ITO test */
extern int gesture_flag;
static bool is_pd_with_guesture = false;
//#endif

#ifdef ODM_HQ_EDIT
/* sunshiyue, 2020/05/20, Add for reset sequence */
...
//#ifdef ODM_HQ_EDIT
/* dongfeiju, 2020/04/22, Add for TP resume */
...
/* sunshiyue, 2020/05/20, Add for reset sequence */
	if (strstr(saved_command_line, "mdss_dsi_nt36525b_90hz_inx_video")) {
		...
		for (i = 0; i < r_config->count; i++) {
			...
			if (r_config->sequence[i].sleep_ms)
				usleep_range(r_config->sequence[i].sleep_ms * 1000, (r_config->sequence[i].sleep_ms * 1000) + 100);
		}

		printk("[lcm] This is novatek, need to do 2 times reset!\n");
	}
#endif
...
/* dongfeiju, 2020/05/13, Add for TP ITO test */
	if(1 == gesture_flag) {
		...
		printk("[touchpanel]enter gestuer mode ,lcm power on");
	} else {
		is_pd_with_guesture = false;
		rc = dsi_pwr_enable_regulator(&panel->power_info, false);
		printk("[touchpanel]exit gestuer mode ,lcm power off");
	}
	if (rc)
		DSI_ERR("[%s] failed to enable vregs, rc=%d\n",
				panel->name, rc);
...
*/
	...
	if (!strcmp(panel->name,"samsung s6e8fc1 samsung amoled fhd+ panel")) {
...
/*Mark.Yao@PSW.MM.Display.LCD.Feature,2019-11-04 add for global hbm */
	...
	oppo_bl_normal_max_level = panel->bl_config.bl_normal_max_level;
...
#ifndef VENDOR_EDIT
/*
 * YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2020/05/25,
...
int dsi_panel_mode_switch_to_cmd(struct dsi_panel *panel)
{
	...
	if (rc)
		DSI_ERR("[%s] failed to send DSI_CMD_SET_CMD_TO_VID_SWITCH cmds, rc=%d\n",
		       panel->name, rc);
#ifdef VENDOR_EDIT
/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2020/04/27, add for porting for ramless oled */
