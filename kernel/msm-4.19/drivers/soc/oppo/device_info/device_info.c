                ...
                size_t count, loff_t *off)
{
	...
	gpio = of_get_named_gpio(np, "sdcard_detect", 0);
	if (gpio < 0) {
		dev_msg("sdcard_detect gpio%d is not specified\n", gpio);
		return -ENODEV;
	}

	value = gpio_get_value(gpio);
	if(value < 0){
		dev_msg("sdcard_detect gpio%d get value failed\n", gpio);
		return -ENODEV;
	}
	...
	if (copy_to_user(buf, page, (len < count ? len : count))) {
		dev_msg("sdcard_detect to user failed\n", gpio);
	    return -EFAULT;
	}
...
static void sdcard_init(struct device_info *dev_info)
{
