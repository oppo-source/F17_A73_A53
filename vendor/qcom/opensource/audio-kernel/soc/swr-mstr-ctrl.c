...
static bool swrm_check_link_status(struct swr_mstr_ctrl *swrm, bool active)
{
	...
	if (retry == 0)
//#ifdef ODM_HQ_EDIT
//liujia@PSW.MM.AudioDriver.AudioParams., 2020/06/16, add log for soundwire link status
		dev_err(swrm->dev, "%s: link status not %s comp_sts:0x%x\n", __func__,
			active ? "connected" : "disconnected", comp_sts);
//#endif /* ODM_HQ_EDIT */
	return ret;
}
...
//wangbin@PSW.MM.AudioDriver.AudioParams., 2020/06/03, add log for headset recogition
	dev_err(swrm->dev, "%s: ssp_period: %d\n", __func__, ssp_period);
//#endif /* ODM_HQ_EDIT */
	/* Clear Rows and Cols */
...
int swrm_wcd_notify(struct platform_device *pdev, u32 id, void *data)
{
	...
	case SWR_DEVICE_SSR_DOWN:
		...
		mutex_unlock(&swrm->devlock);
		#ifdef VENDOR_EDIT
		/*Jianfeng.Qiu@PSW.MM.AudioDriver.Codec, 2020/09/15, Add for qcom patch to fix SSR issue*/
		mutex_lock(&swrm->reslock);
		swrm->state = SWR_MSTR_SSR;
		mutex_unlock(&swrm->reslock);
		#endif /* VENDOR_EDIT */
