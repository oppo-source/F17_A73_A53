...
*/
static unsigned int oppo_nf_hooks_v6_lm_detect(void *priv,
				      struct sk_buff *skb,
				      const struct nf_hook_state *state)
{
    struct nf_conn *ct = NULL;
    ...
    if (is_wechat_skb(ct, skb)) {
        if(skb->protocol == htons(ETH_P_IPV6) && (ipv6h = ipv6_hdr(skb)) != NULL && ipv6h->nexthdr== NEXTHDR_TCP) {
            ...
            for (i = 0; i < wechat_param_count; i++) {
                if (tot_len >= wechat_infos[i].min_tot_len && tot_len <= wechat_infos[i].max_tot_len) {
                    if (memcmp(payload + wechat_infos[i].offset, wechat_infos[i].fixed_value, wechat_infos[i].len) == 0) {
                        printk("oppo_nf_hooks_lm:i=%d received hong bao from ipv6...\n", i);
                        oppo_nf_hooks_send_to_user(NF_HOOKS_LM_DETECTED, NULL, 0);
                        break;
                    } else {
                        if (nf_hooks_debug) printk("oppo_nf_hooks_lm:i=%d fixed value not match from ipv6!!\n", i);
                    }
                } else {
                    if (nf_hooks_debug) printk("oppo_nf_hooks_lm:i=%d incorrect tot_len=%d from ipv6\n", i, tot_len);
                }
            }
        }
    }
...
static struct nf_hook_ops oppo_nf_hooks_ops[] __read_mostly = {
	...
	{
		.hook		= oppo_nf_hooks_v6_lm_detect,
		.pf		    = NFPROTO_IPV6,
		.hooknum	= NF_INET_LOCAL_IN,
