...
__attribute__((weak)) register_devinfo(char *name, struct manufacture_info *info)
{
	return 1;
}
...
//#ifdef ODM_HQ_EDIT
/*zoutao@ODM_HQ.Charge update vooc temp strategy 2020/06/28*/
...
static int oppo_vooc_set_current_1_temp_normal_range(struct oppo_vooc_chip *chip, int vbat_temp_cur)
{
	...
	switch (chip->fastchg_batt_temp_status) {
		case BAT_TEMP_NATURAL:
			#ifdef ODM_HQ_EDIT
			/* Yuzhe.Peng@ODM.BSP.CHARGE 2020/07/13 Add vooc parameters */
			...
			#endif
			// NOTIFY:if (vbat_temp_cur > chip->vooc_strategy1_batt_high_temp0) {
...
*/
	...
	#ifdef ODM_HQ_EDIT
	/* Yuzhe.Peng@ODM_HQ.Charge update vooc temp strategy */
...
out:
	...
	#ifdef ODM_HQ_EDIT
	/* Yuzhe.Peng@ODM_HQ.Charge update vooc temp strategy 2020/07/24 */
	if (chip->fastchg_batt_temp_status == BAT_TEMP_EXIT ) {
		...
		if(!ret) {
			...
			if(chip->vops->fw_check_then_recover) {
				...
				if (chip->vops->is_power_off_charging && chip->vops->is_power_off_charging(chip) == false) {
					...
					chip->fastchg_dummy_started = false;
					#ifdef ODM_HQ_EDIT
					/* Yuzhe Peng@ODM.BSP.Charge vooc firmware update newmethod 2020/07/19 */
					chip->fastchg_started = false;
					chip->allow_reading = true;
					#endif
					del_timer(&chip->watchdog);
