...
#ifndef VENDOR_EDIT
/* Kun.Zhang@PSW.BSP.CHG.Basic, 2020/8/15, rm for huawei a2c rp-rp debug cable charge */
...
/* wangxianfei@ODM.BSP.CHARGE 2020/06/01  oppochg_set_vbus will take over the case */
#ifdef ODM_HQ_EDIT
		break;
#else
...
#ifdef ODM_HQ_EDIT
/* wangxianfei@BSP.CHG.Basic 2020/08/20, 32C~40C charge_done=1 while charging status is FULL. */
...
/* wangxianfei@ODM.BSP.CHARGE 2020/06/01 tuning oppochg_set_vbus */
	...
	if (vbus == VBUS_LIMIT_5V) {
		...
		if (stat & QC_9V_BIT) {
			oppochg_suspend();
			pr_err("%s: set qc to 5V", __func__);
			rc = smblib_masked_write(chg, CMD_HVDCP_2_REG, FORCE_5V_BIT, FORCE_5V_BIT);
			msleep(400);
			oppochg_unsuspend();
		}
		...
		if (stat & QC_5V_BIT) {
			pr_err("%s: set qc to 9V", __func__);
...
static void oppochg_set_usb_mid_boost(bool boost)
{
	struct oppo_chg_chip *chip = g_oppo_chip;
	int rc;
...
/*endif*/
	...
	//#ifdef ODM_HQ_EDIT
	/*zoutao@ODM_HQ.Charge add mid boost 2020/07/24*/
		.set_usb_mid_boost = oppochg_set_usb_mid_boost,
	//#endif
};
#endif
