...
int oppo_chg_parse_charger_dt(struct oppo_chg_chip *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,qc_temp_hot_fastchg_ma_led_on",
			&chip->limits.qc_temp_hot_fastchg_ma_led_on);
	if (rc < 0) {
		chip->limits.qc_temp_hot_fastchg_ma_led_on
			= chip->limits.temp_normal_fastchg_current_ma;
	}
	...
	#ifdef ODM_HQ_EDIT
	/* Yuzhe.Peng@ODM.BSP.CHARGE 2020/07/3 modify for Charge Spec V3.4 */
...
{
	int current_limit = 0;
/* wangxianfei@ODM.BSP.CHARGE 2020/06/05 tuning vbus_limit */
#ifdef ODM_HQ_EDIT
	...
	if ((chip->chg_ctrl_by_lcd) && (chip->led_on)  && !chip->cool_down && !chip->cool_down_9v2a) {
		...
		//#ifdef ODM_HQ_EDIT
		/*zoutao@ODM_HQ.Charge not limit vbus when led on 2020/06/30*/
		//vbus_limit = VBUS_LIMIT_5V;
		//#endif /*ODM_HQ_EDIT*/
...
#ifdef ODM_HQ_EDIT
	/* wangxianfei@ODM.BSP.CHARGE 2020/05/22  priority is given to set_vbus due to aicl_point */
...
static bool oppo_chg_check_vchg_is_good(struct oppo_chg_chip *chip)
{
	...
	if (chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_QC) {
		...
		} else  if (chg_volt < 7600 && chg_volt > 5800) {
			vchg_counts = 0;
			ret = false;
...
//#ifdef ODM_HQ_EDIT
/*zoutao@ODM_HQ.Charge add flash led status 2020/07/23*/
void oppo_chg_set_flash_led_status(bool val)
{
	if (!g_charger_chip)
		return;
...
EXPORT_SYMBOL(oppo_chg_set_flash_led_status);
//#endif
...
void oppo_chg_variables_reset(struct oppo_chg_chip *chip, bool in)
{
	...
	chip->vbatt_over = 0;
#ifdef ODM_HQ_EDIT
	/*Yi.Zhou@ODM.HQ.BSP.CHG.Basic 2020/07/30 fix QC select current*/
	chip->vbat_4180mv_status_changed = true;
#endif
	chip->total_time = 0;
	...
	chip->notify_flag = 0;
#ifdef ODM_HQ_EDIT
/*Yi.Zhou@ODM.HQ.BSP.CHG.Basic, 2020/7/29 update cool down reset*/
chg_err("chip->charger_type =  %d\n",chip->charger_type);
	if (!((chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP)
				|| (oppo_vooc_get_fastchg_started() == true)
				|| (oppo_vooc_get_fastchg_to_warm() == true)
				|| (oppo_vooc_get_adapter_update_status() == ADAPTER_FW_NEED_UPDATE)
...
static void oppo_chg_get_battery_data(struct oppo_chg_chip *chip)
{
	...
	if(oppo_gauge_get_healthd_ready() == 0 && retry_counts < RETRY_COUNTS){
...
static void oppo_chg_check_aicl_input_limit(struct oppo_chg_chip *chip)
{
	...
	if (chip->charging_state == CHARGING_STATUS_FAIL || chip->batt_full == true
			|| ((chip->tbatt_status != BATTERY_STATUS__NORMAL)
			&& (chip->tbatt_status != BATTERY_STATUS__LITTLE_COOL_TEMP))
			|| ((chip->ui_soc >= 80) && (chip->pmic_spmi.aicl_suspend == false))
			|| oppo_vooc_get_fastchg_started() == true) {
		aicl_delay_count = 0;
		return;
	}
...
static void oppo_chg_ibatt_check_and_set(struct oppo_chg_chip *chip)
{
	...
	if (current_adapt == 0) {
		if (chip->tbatt_status == BATTERY_STATUS__LITTLE_COLD_TEMP) {
			...
			if (chip->batt_volt > 4180) {
				current_adapt = chip->limits.temp_little_cool_fastchg_current_ma_low;
			} else {
				current_adapt = chip->limits.temp_little_cool_fastchg_current_ma_high;
			}
	...
	if (chip->tbatt_status == BATTERY_STATUS__LITTLE_COLD_TEMP) {
		...
		if (vbatt_higherthan_4180mv) {
				current_init = chip->limits.temp_little_cool_fastchg_current_ma_low;
				current_limit = chip->batt_capacity_mah * 45 / 100;
		} else {
				current_init = chip->limits.temp_little_cool_fastchg_current_ma_high;
				current_limit = chip->batt_capacity_mah * 45 / 100;
		}
	...
	if(ePreTempStatus != chip->eTempStatus)
	{
		...
		else if(iPreTemp > batt_temp )
		{
			...
			if(iTempDownCount > DOWNCOUNT)
			{
				...
				{
					isChangeStatus = ((batt_temp < (g_arryQcTmpStep[chip->eTempStatus] - QC_TEMP_JEITA)||((batt_temp < g_arryQcTmpStep[chip->eTempStatus]) && g_arryQcTmpStep[chip->eTempStatus] == chip->limits.qc_temp_normal_decidegc)
					|| (chip->qc_lower_fcc_noshake_support && batt_temp <= g_arryQcTmpStep[QC_TEMP_STATUS__COOL])) && batt_temp >= (g_arryQcTmpStep[chip->eTempStatus-1]))? true: false;
				}else {
					isChangeStatus = ((batt_temp < (g_arryQcTmpStep[chip->eTempStatus] - QC_TEMP_JEITA) || (chip->qc_lower_fcc_noshake_support &&
					batt_temp <= g_arryQcTmpStep[QC_TEMP_STATUS__COOL])) && batt_temp >= (g_arryQcTmpStep[chip->eTempStatus-1]))? true: false;
				}
				chg_debug("***iPreTemp:%d,batt_temp%d,[g_arryQcTmpStep[chip->eTempStatus] - QC_TEMP_JEITA]%d,[g_arryQcTmpStep[chip->eTempStatus - 1]]=%d,ischange=%d***\n",
					iPreTemp,batt_temp,g_arryQcTmpStep[chip->eTempStatus] - QC_TEMP_JEITA,g_arryQcTmpStep[chip->eTempStatus - 1],isChangeStatus);
				//***iPreTemp:220,batt_temp214,[g_arryQcTmpStep[chip->eTempStatus] - QC_TEMP_JEITA]210,[g_arryQcTmpStep[chip->eTempStatus - 1]]=-1000,ischange=0***<5>[ 3097.096812]  (0)[22791:kworker/0:1][OPPO_CHG][oppo_chg_get_qc_tbatt_st
			}
			else
			{
				isStatusHold = false;
...
{
#ifdef ODM_HQ_EDIT
/* wangxianfei@BSP.CHG.Basic 2020/07/30, 32C~40C qc charger for tbat rising feature */
...
void oppo_chg_enable_charge(void)
{
	if (!g_charger_chip) {
		return;
	}
	oppo_chg_turn_on_charging(g_charger_chip);
}
...
void oppo_chg_set_usb_mid_boost(bool boost)
{
	if (!g_charger_chip) {
		return;
	} else {
...
void oppo_smart_charge_by_cool_down(struct oppo_chg_chip *chip, int val)
{
	...
	charger_xlog_printk(CHG_LOG_CRTI, "val->intval = [%04x], set cool_down = [%d], cool_down_done=%d, cool_down_9v2a = %d\n", 
		val, chip->cool_down, chip->cool_down_done, chip->cool_down_9v2a);
}
