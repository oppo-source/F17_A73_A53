...
static void init_parse_dts(struct device *dev, struct touchpanel_data *ts)
{
    ...
    if ( g_chip_name == ILI9881H) {
        rc = of_property_read_string(np, "chip-name0", &ts->panel_data.chip_name);
        } else if(g_chip_name == ILI9882N) {
        rc = of_property_read_string(np, "chip-name1", &ts->panel_data.chip_name);
        } else {
...
#define MATCH_GOODIX_9886       "Goodix-gt9886"
...
#ifdef CONFIG_TOUCHPANEL_GOODIX_GT9886
static struct of_device_id tp_match_table_goodix[] = {
    { .compatible = MATCH_GOODIX_9886,},
    { },
};
#endif
...
#ifdef CONFIG_TOUCHPANEL_GOODIX_GT9886
extern int __maybe_unused goodix_tp_probe(struct i2c_client *client, const struct i2c_device_id *id);
extern int __maybe_unused goodix_tp_remove(struct i2c_client *client);
#endif
...
#ifdef CONFIG_TOUCHPANEL_GOODIX_GT9886
    case GT9886:
        tp_i2c_driver.driver.of_match_table = tp_match_table_goodix;
        tp_i2c_driver.probe = goodix_tp_probe;
        tp_i2c_driver.remove = goodix_tp_remove;
        break;
#endif
    default:
